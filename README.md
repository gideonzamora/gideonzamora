## About me 
Hi! I'm Gideon. I am passionate about sharing my career development in the tech industry.<br/>

- Experienced in querying data using SQL to extract meaningful insights.
- Create compelling data visualizations and dashboards in Tableau.
- Data analysis and reporting using Google Sheets and Excel.

### 🎨 Projects

This is where I showcase my [projects](https://github.com/gideonzamora/first-project/blob/main/README.md)

### 🔧 Tools

- Database: MySQL
- Visualization: Tableau, PowerBI, Microsoft Excel

### 💼 Connect with Me

- [Linkedin](https://www.linkedin.com/in/gideon-zamora-844020217/)
- gideon.zamora@gmail.com
